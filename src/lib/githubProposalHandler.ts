...
export class GithubProposalHandler {
  ...
  protected draftdatabasePath = 'DRAFT_DATABASE.json';
  protected draftdatabaseMdPath = 'DRAFT_DATABASE.md';
  protected proposalTemplate = 'TEMPLATE.md';
  ...
  async getProposalTemplate() {
    return this.GithubAPI.getContent(this.proposalTemplate).then((content) => {
      return content;
    }).catch((e) => {
      Promise.reject(e);
    });
